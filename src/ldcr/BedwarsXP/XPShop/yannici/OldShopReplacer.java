package ldcr.BedwarsXP.XPShop.yannici;
...
public class OldShopReplacer implements Runnable {
	Game game;
	CommandSender s;

	public OldShopReplacer(String e, CommandSender sender) {
		s = sender;
		game = io.github.yannici.bedwars.Main.getInstance().getGameManager()
				.getGame(e);
	}
	...
	public void run() {
		...
		if (Config.Add_Res_Shop) {
			...
			trades.add(new XPVillagerTrade(RessourceSpawner
					.createSpawnerStackByConfig(io.github.yannici.bedwars.Main
							.getInstance().getConfig().get("ressource.bronze"))));
			trades.add(new XPVillagerTrade(RessourceSpawner
					.createSpawnerStackByConfig(io.github.yannici.bedwars.Main
							.getInstance().getConfig().get("ressource.iron"))));
			trades.add(new XPVillagerTrade(RessourceSpawner
					.createSpawnerStackByConfig(io.github.yannici.bedwars.Main
							.getInstance().getConfig().get("ressource.gold"))));
