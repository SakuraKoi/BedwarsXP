...
public class EditXPCommandListener implements CommandExecutor {
	...
	public boolean onCommand(final CommandSender sender, final Command arg1, final String arg2, final String[] args) {
		if (args.length == 0) {
			SendMessageUtils.sendMessage(sender, "§6§lBedwarsXP §7>> §b经验起床插件 §lBy.Ldcr", "§6§lBedwarsXP §7>> §a/editxp <经验值>  设置自己的经验值", "§6§lBedwarsXP §7>> §a/editxp <玩家> <经验值> 设置其他玩家的经验值");
			return true;
		}
		...
		if (offPlayer != null) {
			...
			if (offPlayer.isOnline()) {
				...
				if (bw == null) {
					sender.sendMessage("§6§lBedwarsXP §7>> §c玩家 " + p.getName() + " 不在游戏中!");
					return true;
				}
				if (!Config.isGameEnabledXP(bw.getName())) {
					sender.sendMessage("§6§lBedwarsXP §7>> §c玩家 " + p.getName() + " 所在的游戏没有开启经验起床模式!");
					return true;
				}
				final int xp;
				try {
					...
					sender.sendMessage("§6§lBedwarsXP §7>> §c输入经验值的不是一个有效数字!");
					return true;
				}
				XPManager.getXPManager(bw.getName()).setXP(p, xp);
				sender.sendMessage("§6§lBedwarsXP §7>> §a玩家 " + p.getName() + " 的经验值已被设置为 " + xp);
			} else {
				sender.sendMessage("§6§lBedwarsXP §7>> §c玩家 " + offPlayer.getName() + " 不在线!");
				return true;
			}
			return true;
		}
		if (!(sender instanceof Player)) {
			SendMessageUtils.sendMessage(sender, "§6§lBedwarsXP §7>> §b经验起床插件 §lBy.SakuraKooi", "§6§lBedwarsXP §7>> §a/editxp <玩家> <经验值> 设置玩家的经验值");
			return true;
		}
